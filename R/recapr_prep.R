...
recapr_prep <- function(ID, event=NULL, recap_codes=NULL, ...) {
  ...
  out$meta <- list(ID = c(ID[1], ID[length(ID)]))
  names(out$meta$ID) <- the_events
...
correct_growth <- function(x,
                           ...
                           column_keep=NULL, column_adjust=column_keep,
                           ID_keep=NULL, ID_adjust=ID_keep,
                           impute = TRUE) {
  ...
  if(any(length(event_keep)>1,
         length(event_adjust)>1,
         length(column_keep)>1,
         length(column_adjust)>1,
         length(ID_keep)>1,
         length(ID_adjust)>1)) {
    stop("All column identifiers must be length-1")
  }
  ...
  ############  pulling size from $meta
  if(is.null(column_keep) & is.null(column_adjust)) {
    if(is.null(x$meta$size)) stop("columns to keep and adjust must be supplied")
    column_keep <- x$meta$size[[event_keep]]
    column_adjust <- x$meta$size[[event_adjust]]
  }
  ...
  ############  pulling ID from $meta
  if(is.null(ID_keep) & is.null(ID_adjust)) {
    ID_keep <- x$meta$ID[[event_keep]]
    ID_adjust <- x$meta$ID[[event_adjust]]
  }
  ...
  ###### defining a new size column
  # x1$meta$size[[event_keep]] <- column_keep
  # x1$meta$size[[event_adjust]] <- paste(column_adjust, "adjusted", sep="_")
  x1$meta$size <- c(column_keep, paste(column_adjust, "adjusted", sep="_"))
  names(x1$meta$size) <- c(event_keep, event_adjust)
  message(paste0("Size columns set to ",
                 column_keep, " and ", paste(column_adjust, "adjusted", sep="_"), " for ",
                 event_keep, " and ", event_adjust, ", respectively."))
...
truncate <- function(x, event_names=NULL, column_names=NULL, min=NULL, max=NULL) {
  ...
  ######## pulling column_names and event_names from $meta if possible
  if(is.null(column_names)) {  # is.null(event_names) &
    if(is.null(x$meta$size)) stop("Column names and corresponding event names must be supplied")
    column_names <- unname(x$meta$size)
    event_names <- names(x$meta$size)
  }
  ...
  x1$meta$size <- column_names
  names(x1$meta$size) <- event_names
  message(paste0("Size columns set to ",
                 column_names[1], " and ", column_names[2], " for ",
                 event_names[1], " and ", event_names[2], ", respectively."))
  return(x1)
}



stratify <- function(x, event_names=NULL, column_names=NULL, breaks, right=FALSE, dig.lab=6) {
  ...
  x1$meta$strata <- paste0(column_names, "_strat")
  names(x1$meta$strata) <- event_names
  message(paste0("Strata columns set to ",
                 column_names[1], "_strat and ", column_names[2], "_strat for ",
                 event_names[1], " and ", event_names[2], ", respectively."))
...
# column_names = NULL will pull automatically as possible
# column_names = NA will override to no strata
tabulate_samples <- function(x,
                             column_names=NULL,
                             event_names=NULL,
                             suppressNA=FALSE) {   # this will not use NA in counts
  # error checking
  # - needs to be an object returned by recapr_prep
  if(!inherits(x, "MR_data")) stop("Argument x= must be an object returned from recapr_prep()")
  ...
  if(length(column_names) == 1 & all(!is.na(column_names))) {
    column_names <- rep(column_names, 2)
    if(is.null(event_names)) {
      event_names <- names(x$input_data)
    }
  ...
  if(is.null(column_names) & is.null(event_names)) {
    ##### pulling from $strata if possible
    if(!is.null(x$meta$strata)) {
      column_names <- unname(x$meta$strata)
      event_names <- names(x$meta$strata)
    } else {
      event_names <- names(x$input_data)
    }
  }
  if(all(is.na(column_names))) {   # this forces no strata
    event_names <- names(x$input_data)
    column_names <- NULL
  }
